...
use crate::tokens::{Mim, BlockId, DelayState, PageId, SegmentId, range_block_ids};
use crate::types::*;
use crate::config::*;
use crate::layout::*;
            ...
            == old(local).thread_token.value().pages[page_ptr.page_id@].num_blocks + extend as nat,
{
    ...
    let ghost mem_before_take = local.segments[segment_id].mem;
    ...
    proof {
        ...
        assert(mem_before_take == old(local).segments[segment_id].mem);
        assert(local.segments[segment_id].mem.points_to.dom()
            == mem_before_take.points_to.dom().difference(set_int_range(rng_start, rng_start + rng_size)));
        assert(local.segments[segment_id].mem.os == mem_before_take.os);
        assert(local.segments[segment_id].mem.points_to.provenance() == mem_before_take.points_to.provenance());
    }
    ...
    proof {
        ...
        by {
                ...
                assert(local.segments[sid].mem.os == old(local).segments[sid].mem.os);
                assert(local.segments[sid].mem.points_to.provenance()
                    == old(local).segments[sid].mem.points_to.provenance());
                ...
                assert forall |pid: PageId| #[trigger] local.page_organization.pages.dom().contains(pid) implies
                    local.page_count(pid) == old(local).page_count(pid) by {
                    if pid == page_id {
                        assert(local.pages[pid].count == old(local).pages[pid].count);
                    } else {
                ...
                assert forall |pid: PageId| #[trigger] local.page_organization.pages.dom().contains(pid) implies
                    ...
                    if pid == page_id {
                        assert(local.block_size(pid) == old(local).block_size(pid));
                    } else {
                        assert(local.pages[pid] == old(local).pages[pid]);
                    }
                };
                assert forall |pid: PageId| #[trigger] local.page_organization.pages.dom().contains(pid) && pid != page_id implies
                    local.page_capacity(pid) == old(local).page_capacity(pid) by {
                    assert(local.pages[pid] == old(local).pages[pid]);
                };
                assert(old(local).page_capacity(page_id) <= local.page_capacity(page_id));
                local.segment_page_totals_preserved_by_capacity_increase(*old(local), sid, page_id);
                assert(old(local).segment_pages_range_total(sid) <= local.segment_pages_range_total(sid));
                assert(local.segment_pages_used_total(sid) =~= old(local).segment_pages_used_total(sid));
                if sid == segment_id {
                    assert(old(local).page_capacity(page_id) == capacity as int);
                    assert(old(local).block_size(page_id) == bsize as int);
                    assert(rng_start == block_start_at(page_id, bsize as int, capacity as int));
                    assert(rng_start == block_start_at(page_id, old(local).block_size(page_id), old(local).page_capacity(page_id)));
                    assert(rng_start + rng_size == block_start_at(page_id, bsize as int, capacity as int + extend as int)) by(nonlinear_arith)
                        requires
                            rng_start == page_start(page_id) + start_offset(bsize as int) + capacity as int * bsize as int,
                            rng_size == extend as int * bsize as int;
                    assert(block_start_at(page_id, local.block_size(page_id), local.page_capacity(page_id))
                        == block_start_at(page_id, bsize as int, capacity as int + extend as int));
                    assert(rng_start + rng_size == block_start_at(page_id, local.block_size(page_id), local.page_capacity(page_id)));
                    assert(old(local).segments[sid].mem.points_to.dom()
                        <= local.segments[sid].mem.points_to.dom()
                            + set_int_range(
                                block_start_at(page_id, old(local).block_size(page_id), old(local).page_capacity(page_id)),
                                block_start_at(page_id, local.block_size(page_id), local.page_capacity(page_id)))) by {
                        assert forall |addr: int| #[trigger] old(local).segments[sid].mem.points_to.dom().contains(addr) implies
                            (local.segments[sid].mem.points_to.dom()
                                + set_int_range(
                                    block_start_at(page_id, old(local).block_size(page_id), old(local).page_capacity(page_id)),
                                    block_start_at(page_id, local.block_size(page_id), local.page_capacity(page_id)))).contains(addr) by {
                            if local.segments[sid].mem.points_to.dom().contains(addr) {
                            } else {
                                assert(local.segments[sid].mem.points_to.dom()
                                    == old(local).segments[sid].mem.points_to.dom().difference(set_int_range(rng_start, rng_start + rng_size)));
                                assert(set_int_range(rng_start, rng_start + rng_size).contains(addr));
                            }
                        };
                    };
                    local.page_capacity_increase_covers_points_to(*old(local), sid, page_id);
                } else {
                    assert(old(local).segments[sid].mem.points_to.dom()
                        <= local.segments[sid].mem.points_to.dom() + local.segment_pages_range_total(sid)) by {
                        assert forall |addr: int| #[trigger] old(local).segments[sid].mem.points_to.dom().contains(addr) implies
                            (local.segments[sid].mem.points_to.dom() + local.segment_pages_range_total(sid)).contains(addr) by {
                            assert(local.segments[sid].mem.points_to.dom().contains(addr));
                        };
                    };
                }
                local.mem_chunk_good_preserved_by_range_transfer(*old(local), sid);
        };
        if old(local).wf() {
            assert(local.wf_main());
            assert(local.page_organization.popped == crate::page_organization::Popped::No);
            assert(local.wf());
        }
    }
...
pub fn page_extend_free(
    ...
    requires
        ...
        block_start_at(
            page_ptr.page_id@,
            old(local).block_size(page_ptr.page_id@),
            old(local).page_reserved(page_ptr.page_id@))
            <= segment_start(page_ptr.page_id@.segment_id) + SEGMENT_SIZE,
        old(local).segments[page_ptr.page_id@.segment_id].mem.pointsto_has_range(
            block_start_at(
                ...
                old(local).block_size(page_ptr.page_id@),
                old(local).page_capacity(page_ptr.page_id@)),
            (old(local).page_reserved(page_ptr.page_id@) - old(local).page_capacity(page_ptr.page_id@))
                * old(local).block_size(page_ptr.page_id@)),
    ensures
        common_preserves(*old(local), *final(local)),
        old(local).wf() ==> final(local).wf(),
